...
/**
 * Base helper to create a styled embed
 */
 ...
 */
export function createWarningEmbed(title: string, description: string): EmbedOptions {
    return createBaseEmbed(title, description, 0xFEE75C);
}
...
export function itemList(items: IdleItem[]) {
    ...
    const content = items.map(item => {
        ...
        if (Array.isArray(item.percentHistory) && item.percentHistory.length > 0) {
            ...
            if (uniquePercent !== undefined) {
                if (typeof uniquePercent === 'number') {
                    const isPeak = item.percent > uniquePercent;
                    const nextExtremaPercent = reversedPercentHistory.find((percent, i) =>
                        i > reversedPercentHistory.indexOf(uniquePercent) &&
                        item.percent !== null &&
                        typeof percent === 'number' &&
                        (isPeak ? percent >= item.percent : percent <= item.percent)
                    );
                    const days = nextExtremaPercent !== undefined
                        ? reversedPercentHistory.indexOf(nextExtremaPercent, reversedPercentHistory.indexOf(uniquePercent))
                        : reversedPercentHistory.length;
...
function autoCategory(commandName: string): "General" | "Bot" | "Geoguesser" {
    const general = ["help", "avatar", "remind", "channel","waifu"];
    const bot = ["gcalc", "profile", "infopush","voidcook","infopush","market","packing"];
    const geoguesser = ["flag","capital","united states","kabupaten","province","language","aliases","ascend","shop","top"];

    if (general.includes(commandName)) return "General";
    if (bot.includes(commandName)) return "Bot";
    if (geoguesser.includes(commandName)) return "Geoguesser";
    return "General";
}
...
export function commandListEmbed(): EmbedOptions {
    ...
    .map(([cat, list]) => ({
        name: cat === "General" ? " General" : cat === "Bot" ? " Bot" : " Geoguesser",
        value: list.join("\n"),
        inline: false
    }));
