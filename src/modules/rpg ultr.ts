...
client.on('messageCreate', async message => {
    ...
    const hasDoubleButton = Array.isArray(message.components) &&
        message.components.length > 0 &&
        Array.isArray((message.components[0] as any).components) &&
        (message.components[0] as any).components.length === 4;
